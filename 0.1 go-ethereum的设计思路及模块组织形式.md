...
这些交易被打包到一个区块(Block)中，每个区块都和之前的区块链接起来，形成一个反向链表，所以叫区块链。
...
难免会产生多条路径，就是所谓的分叉。因为成功证实了一个新区块会得到一定以太币的奖励，所以从经济学和博弈论的角度，选择主链是最优的。
...
![image](https://github.com/toints/Ethereum-Source-Analysis/blob/master/0.imgs/blockchain-frame.png)
...
### 1. 区块(Block)是以太坊的核心数据结构之一，Block由包含Header和Body两部分。
